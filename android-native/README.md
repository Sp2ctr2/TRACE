...
Canonical coral is retained for the mark. Buttons use the canonical deep coral `#D93B25` against white (calculated contrast approximately 4.56:1). Small text uses `#676960` against paper (approximately 5.07:1), slightly darker than the site's original muted token. Platform sans-serif/Korean fallback is used; no font files or runtime font download are included.
