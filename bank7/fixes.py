...
# The compact task preserves payee, amount, all detail rows and primary action.
# Only the repeated question and surplus gap are omitted, not smaller text/targets.
...
# Reserve the navigation physically, so ScrollTo and TalkBack focus cannot stop
# underneath the overlaid dock. Additional bottom padding is no substitute.
...
# Some Material modal windows update the activity insets during disposal. Restore
# desired flags on the next focused draw, without restarting edge-to-edge each frame.
...
print('Fixed compact review fit, real navigation reservation, reissue binding and modal/theme system contrast.')
